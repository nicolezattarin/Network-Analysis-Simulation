...
for aa in a:
    np.random.seed(0)
    outage_6dB.append(GQR_cellular_system(r0, R, SIR_threshold=6, alpha=aa, sigma=8, eta=4, R1=R1, R2=R2)['failure_prob'])
    outage_10dB.append(GQR_cellular_system(r0, R, SIR_threshold=10, alpha=aa, sigma=8, eta=4, R1=R1, R2=R2)['failure_prob'])
...
for gg in G:
    np.random.seed(0)
    throughput_6dB.append(GQRmulti_access(r0, 1, SIR_threshold=6, G=gg, sigma=8)['success_prob'])
    throughput_10dB.append(GQRmulti_access(r0, 1, SIR_threshold=10, G=gg, sigma=8)['success_prob'])
